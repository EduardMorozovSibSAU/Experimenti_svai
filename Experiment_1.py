...
class Config:
    ...
    RESULTS_FOLDER: str = "Results_experiment_1_newscript" # название папки для сохранения моделей, графиков и итогов (создаётся автоматически)
    ...
    TARGET_IS_ABSOLUTE: bool = False # флаг для абсолютных значений целевой переменной
...
def filter_outliers_by_quantile(df: pd.DataFrame, cols: List[str],  low: float = 0.02,  high: float = 0.98) -> pd.DataFrame:
    ...
    lower = df[cols].quantile(low)
    upper = df[cols].quantile(high)
    mask = (df[cols] >= lower).all(axis=1) & (df[cols] <= upper).all(axis=1)
    return df[mask].copy()
...
def load_temperature_growth(filepath: str, sheet_name: str = None) -> pd.DataFrame:
    ...
    df_temp_new.drop(0, inplace=True) # удаляем строку с заголовками
    df_temp_new.columns = ['Объект', 'Участок', 'TS_num', 'Имя ТС','Depth','I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII', 'XIII'] # 'I',
    df_temp_new

    id_cols = ['Объект', 'Участок', 'TS_num', 'Имя ТС', 'Depth']

    df_long = df_temp_new.melt( # плавим столбцы с циклами в один столбец "Цикл" и значения в столбец "value" для того чтобы было как в исходном формате
        id_vars=id_cols,
        var_name='Цикл',
        value_name='value'
    )

    df_final = df_long.pivot_table( # разворачиваем обратно, но уже с циклами в виде отдельных столбцов и значениями температуры в этих столбцах
        index=['TS_num', 'Цикл'],
        columns='Depth',
        values='value',
        aggfunc='first'
    ...
    df_temp_new = df_final.rename(columns=lambda x: f'temp_{int(x)}' if isinstance(x, (int, float)) else x) # переназывает колонки
    ...
    if cfg.TEMPERATURE_IS_ABSOLUTE:
        df_temp_new[["temp_6", "temp_7","temp_8", "temp_9", "temp_10"]] = df_temp_new[["temp_6", "temp_7","temp_8", "temp_9", "temp_10"]].apply(abs)

    print(df_temp_new.columns)
    return df_temp_new
...
def load_swamp_percent(filepath: str):
    df = pd.read_excel(filepath)
    df = df[['Номер участка','% заболоченности территории']]
...
def scale_all_data(dataframe):
    ...
    scale_cols = [
        *names_di_columns, *snow_presc_temp, *[c for group in temp_cols for c in group], 'Distance']  
...
class GTMDataset(Dataset):
    def __init__(self, data: pd.DataFrame, target_is_absolute: bool = False, swamp_percent: bool = False):
        super().__init__()
...
def main(exp_id: int = 1):
    ...
    # в 1, 2, 4, 5, 9, 10 экспериментах использовать "ДМ_перемещения приростом"  из файла "new_data.xlsx" 
    # в 3, 6, 7, 8 экспериментах использовать "Перемещения"  из файла "Все параметры_12 циклов_new_26.12.xlsx" 
    ...
    # temp_df = load_temperature_growth("new_data.xlsx", "ТС_тренд приростом") # 11 эксперимент - ДМ_ перемещения приростом ,  Тренд температуры приростом
    # temp_df = load_temperature_growth("new_data.xlsx", "ТС_тренд приростом") # 12 эксперимент - ДМ_ перемещения приростом ,  Тренд температуры приростом + cfg.TEMPERATURE_IS_ABSOLUTE = True
    # temp_df = load_temperature_growth("new_data.xlsx", "ТС_тренд последовательный") # 13 эксперимент - ДМ_ перемещения приростом ,  Тренд температуры последовательный 
    # temp_df = load_temperature_growth("new_data.xlsx", "ТС_тренд последовательный") # 14 эксперимент - ДМ_ перемещения приростом ,  Тренд температуры последовательный + cfg.TEMPERATURE_IS_ABSOLUTE = True
    temp_df = load_temperature("Все параметры_12 циклов_new_26.12.xlsx", "Температура_new") # в 1 - 10 экспериментах использовать "Температура_new"  из файла "Все параметры_12 циклов_new_26.12.xlsx"
    ...
    train_groups = prepare_data(cfg.ROOT / cfg.DATA_FILE, cycles_to_include=["II","III","IV","V","VI","VII","VIII","IX","X", "XIV"])
